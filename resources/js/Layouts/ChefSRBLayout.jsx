...
function ChefSRBLayout({ children }) {
    ...
    const getActiveLink = () => {
        if (current_route.includes("dashboard")) {
            ...
            current_route.includes("intern") &&
            !current_route.includes("internships")
        ) {
            return "interns";
    ...
    const childrenWithProps = React.Children.map(children, (child) => {
        if (React.isValidElement(child)) {
            return React.cloneElement(child, { ohatra: ohatra });
        }
        return child;
    });
            ...
            <Authenticated>
                <SideBarContents>
                    <SideBarLinks
                        ...
                        text={"Dashboard"}
                        active={active_link == "dashboard"}
                        href={"chefSRB.dashboard"}
                        alert
                        notifs={"2+"}
                    />
                    <SideBarLinks
                        ...
                        text="Interviews"
                        active={active_link == "interview"}
                        href={"chefSRB.interview"}
                    />
                    <SideBarLinks
                        icon={<NotebookText size={22} />}
                        text="Offers"
                        active={active_link == "offers"}
                        href={"chefSRB.offers"}
                    />
                    <SideBarLinks
                        icon={<BookUser size={22} />}
                        text="Interns"
                        active={active_link == "interns"}
                        href={"chefSRB.interns"}
                    />
                    <SideBarLinks
                        icon={<GraduationCap size={22} />}
                        text="InternShips"
                        active={active_link == "internships"}
                        href={"chefSRB.internships"}
                    />
                    <SideBarLinks
                        icon={<Users size={22} />}
                        text="Accounts"
                        active={active_link == "accounts"}
                        href={"chefSRB.accounts"}
                    />
                    <SideBarLinks
                        icon={<Workflow size={22} />}
                        text="Work Units"
                        active={active_link == "units"}
                        href={"chefSRB.units"}
                    />
                </SideBarContents>
